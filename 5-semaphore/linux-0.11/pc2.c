...
int main(void)
{
	...
	int res;
	/* 打开文件 */
	int bf = open("buffer.txt", O_CREAT|O_TRUNC|O_RDWR, S_IRUSR|S_IWUSR);
	if (bf == -1) {
	...
	sem_t *sem_empty = sem_open("empty", O_CREAT|O_EXCL, S_IRUSR|S_IWUSR, SIZE);
	if (sem_empty == SEM_FAILED) {
		perror("Create semaphore empty failed!");
		exit(EXIT_FAILURE);
	}
	sem_t *sem_full = sem_open("full", O_CREAT|O_EXCL, S_IRUSR|S_IWUSR, 0);
	if (sem_full == SEM_FAILED) {
		perror("Create semaphore full failed!");
		exit(EXIT_FAILURE);
	}
	sem_t *sem_mutex = sem_open("mutex", O_CREAT|O_EXCL, S_IRUSR|S_IWUSR, 1);
	if (sem_mutex == SEM_FAILED) {
		perror("Create semaphore mutex failed!");
		exit(EXIT_FAILURE);
	}
	sem_t *sem_atom = sem_open("atom", O_CREAT|O_EXCL, S_IRUSR|S_IWUSR, 1);
	...
	if ((pid=fork()) == -1) {
		...
		if ((pid=fork()) == -1) {
			perror("Consumer 1 fork failed!");
			exit(EXIT_FAILURE);
		} else if (!pid) {	// consumer 1
			do {
				sem_wait(sem_atom);
				//读取上次写入的数值
				sem_wait(sem_mutex);
			...
			if ((pid=fork()) == -1) {
				perror("Consumer 2 fork failed!");
				exit(EXIT_FAILURE);
			} else if (!pid) {	// Consumer 2
				do {
					sem_wait(sem_atom);
					//读取上次写入的数值
					sem_wait(sem_mutex);
					res = lseek(bf, 0, SEEK_SET);
					if (res == -1) {
						perror("Consumer 2 lseek failed!");
						exit(EXIT_FAILURE);
					}
					if ((res = read(bf, &val, sizeof(val))) == -1) {
						perror("Consumer 2 read failed!");
						exit(EXIT_FAILURE);
					}
					sem_post(sem_mutex);
					if (val < M) {
						...
						if ((res = write(bf, &val, sizeof(val))) == -1) {
							perror("Consumer 2 write failed!");
							exit(EXIT_FAILURE);
						}
						sem_post(sem_mutex);
					}
